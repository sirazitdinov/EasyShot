...
// Кэш для отслеживания инициализированных вкладок
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    const handleRequest = async () => {
        try {
            ...
            if (request.status === 'RULER_INITIALIZED' && sender.tab) {
                initializedTabs.add(sender.tab.id);
                console.log(`Пиксельная линейка инициализирована для вкладки ${sender.tab.id}`);
                return { success: true }; // Возвращаем успех и завершаем обработку
            }
...
// Очищаем кэш при закрытии вкладки
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            if (tabs[0] && tabs[0].id) {
                ...
                if(initializedTabs.has(tabId)){
                    // Отправка сообщения в предварительно загруженный content-script
                    chrome.tabs.sendMessage(tabId, { action: "toggleRuler" });
                } else {
                    // Fallback: пытаемся отправить сообщение (на случай если скрипт загружен но не успел инициализироваться)
                    chrome.tabs.sendMessage(tabId, {action: "toggleRuler"}, (response) => {
                        if (chrome.runtime.lastError) {
                            console.log('Content script not ready, injecting...');
                            // Если скрипт не отвечает, загружаем его
                            chrome.scripting.executeScript({
                                target: {tabId},
                                files: ['content.js']
                            }).then(() => {
